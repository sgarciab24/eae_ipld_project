...
def load_data():
    ...
    temps_df = pd.read_csv("data/cities_temperatures.csv")  # TODO: Ex 3.1: Load the dataset using Pandas, use the data_path variable and set the index column to "show_id"
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        city_df = pd.DataFrame(temps_df[temps_df["City"]== city])            # TODO
        city_df_period = pd.DataFrame(city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)])    # TODO
        plt.plot(city_df_period["Date"], city_df_period["AvgTemperatureCelsius"], label=city)                 # TODO 
    plt.title(f"Average Temperature in Celsius for {', '.join(selected_cities)}")   # TODO
    ...
    for city in selected_cities:
        city_df = pd.DataFrame(temps_df[temps_df["City"]== city])             # TODO
        city_df_period = pd.DataFrame(city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)])      # TODO
        plt.hist(city_df_period["AvgTemperatureCelsius"], label=city, bins=20, alpha=0.7,edgecolor="black")                # TODO

    plt.title(f"Average Temperature (ºC) Distribution for {', '.join(selected_cities)}")   # TODO
    plt.xlabel("Temperature (ºC)")  # TODO
